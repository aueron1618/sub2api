...
/**
 * Complete Discord OAuth registration by supplying an invitation code
 * @param pendingOAuthToken - Short-lived JWT from the OAuth callback
 * @param invitationCode - Invitation code entered by the user
 * @returns Token pair on success
 */
export async function completeDiscordOAuthRegistration(
  pendingOAuthToken: string,
  invitationCode: string
): Promise<{ access_token: string; refresh_token: string; expires_in: number; token_type: string }> {
  const { data } = await apiClient.post<{
    ...
    expires_in: number
    token_type: string
  }>('/auth/oauth/discord/complete-registration', {
    pending_oauth_token: pendingOAuthToken,
    invitation_code: invitationCode
  })
  return data
