    ...
    <div class="space-y-6">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.discord.callbackTitle') }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ isProcessing ? t('auth.discord.callbackProcessing') : t('auth.discord.callbackHint') }}
        </p>
      </div>
      ...
      <transition name="fade">
        <div v-if="needsInvitation" class="space-y-4">
          <p class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('auth.discord.invitationRequired') }}
          </p>
          <div>
            <input
          ...
          >
            {{ isSubmitting ? t('auth.discord.completing') : t('auth.discord.completeRegistration') }}
          </button>
        </div>
      </transition>
...
import { completeDiscordOAuthRegistration } from '@/api/auth'
...
async function handleSubmitInvitation() {
  ...
  try {
    const tokenData = await completeDiscordOAuthRegistration(
    ...
    invitationError.value = buildAuthErrorMessage(error, {
      fallback: t('auth.discord.completeRegistrationFailed'),
      t
    })
  } finally {
...
onMounted(async () => {
  ...
  if (error) {
    if (error === 'invitation_required') {
      ...
      if (!pendingOAuthToken.value) {
        errorMessage.value = t('auth.discord.invalidPendingToken')
    ...
    errorMessage.value = resolveOAuthCallbackErrorMessage({
      ...
      fallback: t('auth.discord.errors.loginFailed'),
      t
    })
    appStore.showError(errorMessage.value)
    isProcessing.value = false
  ...
  if (!token) {
    errorMessage.value = t('auth.discord.callbackMissingToken')
    appStore.showError(errorMessage.value)
    isProcessing.value = false
    return
  }
